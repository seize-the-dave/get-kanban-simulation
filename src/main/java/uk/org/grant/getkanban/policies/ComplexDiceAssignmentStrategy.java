...
public class ComplexDiceAssignmentStrategy implements DiceAssignmentStrategy {
    ...
    public void assignDice(Board board) {
        ...
        for (State state : new State[]{State.TEST, State.DEVELOPMENT, State.ANALYSIS, State.TEST, State.DEVELOPMENT}) {
            ...
            LOGGER.info("Assigning workers for the {} column", state);
            ...
            List<StateDice> allWorkers = diceToAllocate.stream().filter(primary).collect(Collectors.toList());
            ...
            LOGGER.info("Putting {} to work against cards in {}", allWorkers, column);
            ...
            for (Card c : incompleteCards) {
                ...
                int remainingDice = this.maxDice;
                ...
                if (secondaries.size() > 0 && remainingWork.compareTo(BigDecimal.ZERO) == 1) {
                    // Remove the work
        ...
        if (jobSeekers.size() > 0) {
            LOGGER.error("Unused dice: {}", jobSeekers);
        }
    }
}
